...
weeks = 47
N1 = weeks
active_weeks = np.zeros(weeks)
dead_weeks = np.zeros(weeks)
recovered_weeks = np.zeros(weeks)
susceptible_weeks = np.zeros(weeks)

for i in range(weeks):
    active_weeks[i] = active[i*7]
    dead_weeks[i] = dead[i * 7]
    recovered_weeks[i] = recovered[i * 7]
    susceptible_weeks[i] = susceptible[i * 7]

data1 = np.stack((susceptible_weeks , active_weeks, recovered_weeks, dead_weeks), axis=-1)
...
plt.figure(1)
show_cov(sigma_theta, max_abs, dataset, run_id, N1)

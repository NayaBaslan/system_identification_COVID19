...
def sird_model_sym_re_9(x, controls, p,beta_tv):

  dSdt = [] ;  dIdt = [];  dRdt = []; dDdt = []
  for i in range(9):
    S = x[0+i]; I = x[i+9]; R = x[i+2*9]; D = x[i+3*9];
    beta = controls[0:9, i]; gamma = p[0, i]; mu = p[1, i]

    Is = x[9:18, 0]
    N_pop0 = S + I + R + D
    prod = S*beta*Is
    S_sum = sum(vertsplit(prod,1))
    dSdt_i = - beta_tv * S_sum / N_pop0  # -  gamma_0*Is[0] - mu_0*Is[0]
    dIdt_i = beta_tv * S_sum / N_pop0 - gamma * Is[i] - mu * Is[i]
    dRdt_i = gamma * Is[i]
    dDdt_i = mu * Is[i]

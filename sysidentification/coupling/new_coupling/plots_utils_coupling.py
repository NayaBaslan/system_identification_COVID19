...
def plot_active(t, active, age_groups, N):
    ...
    colors = ["navy", "forestgreen", "darkgoldenrod", "brown", "purple","black", "red", "peru", "darkgoldenrod", "olive","darkolivegreen"]
    scaley = 1e3
    for i in range(age_groups):
        # plt.plot(t, active[i, :], label='Age Group %s ' % i)
        plt.plot(t, active[i, :] / scaley, color=colors[i])
        print(i)
    plt.legend(["A 00-09", "A 10-19", "A 20-29", "A 30-39", "A 40-49", "A 50-59", "A 60-69", "A 70-79","A  80+"], fontsize=14)
...
def plot_active_sim(t, active, age_groups, N, bounds):
    plt.title("Total Simulated Active Infections Germany ", fontsize = 14)
    colors = ["navy", "forestgreen", "darkgoldenrod", "brown", "purple","black", "red", "peru", "darkgoldenrod", "olive","darkolivegreen"]
    colors_fill = ["mediumslateblue", "springgreen", "peachpuff", "lightpink", "lavender","pink", "moccasin", "wheat", "beige", "thistle"]
    ...
    for i in range(age_groups):
        ...
        plt.fill_between(t, active[i, :]/ scaley-5*i/50, active[i, :]/ scaley+5*i/50, facecolor=colors_fill[i])
    plt.legend(["A 00-09", "A 10-19", "A 20-29", "A 30-39", "A 40-49", "A 50-59", "A 60-69", "A 70-79","A  80+"], fontsize=14)
    if (N==20):
        plt.xticks(np.arange(1, N +1, step=4), months, rotation=30, fontsize=14)
    elif(N==17):
...
def plot_active_sim_real(t, active_model, active_real, age_groups, N, bounds):
    plt.title("Real vs. Model Active Infections for A00-19 ", fontsize = 14)
    colors = ["navy", "forestgreen", "darkgoldenrod", "brown", "purple","black", "red", "peru", "darkgoldenrod", "olive","darkolivegreen"]
    colors_fill = ["mediumslateblue", "springgreen", "peachpuff", "lightpink", "lavender","pink", "moccasin", "wheat", "beige", "thistle"]
    ...
    fig, axs = plt.subplots(age_groups,  sharex=True, sharey=True)
    fig.suptitle("Real vs. Model Active Infections per Age Group", fontsize = 20)
    fig.text(0.04, 0.5, r'Active Infections $[10^3]$', fontsize=20, va='center', rotation='vertical')
    ...
    # axs[0].set_title('Age Group A00-09')
    # plt.legend(["Simualated Active Infections", "Real Active Infections"], fontsize=14)
    ...
    # ax2 = axs[0].twinx()
    # ax2.tick_params(axis="y", which="both", bottom=False,right=False)
    # ax2.axis('off')
    # ax2.set_ylabel('A 00-09', fontsize=14, rotation =10)

    # axs[1].set_title('Age Group A10-19')
    ...
    # axs[2].set_title('Age Group A20-29')
    ...
    # axs[3].set_title('Age Group A30-39')
    ...
    # axs[4].set_title('Age Group A40-49')

    axs[5].plot(t, active_real[5, :] / scaley, color='darkslategray', label='Age Group %s ' % 0)
    axs[5].plot(t, active_model[5, :] / scaley, color='gold', label='Age Group %s ' % 0)
    axs[5].fill_between(t, active_model[5, :] / scaley - 5 * 4 / 10, active_model[5, :] / scaley + 5 * 4 / 10,
                        facecolor=colors_fill[0])
    # axs[5].set_title('Age Group A50-59')

    axs[6].plot(t, active_real[6, :] / scaley, color='darkslategray', label='Age Group %s ' % 0)
    axs[6].plot(t, active_model[6, :] / scaley, color='gold', label='Age Group %s ' % 0)
    axs[6].fill_between(t, active_model[6, :] / scaley - 5 * 4 / 10, active_model[6, :] / scaley + 5 * 4 / 10,
                        facecolor=colors_fill[0])
    # axs[6].set_title('Age Group A60-69')

    axs[7].plot(t, active_real[7, :] / scaley, color='darkslategray', label='Age Group %s ' % 0)
    axs[7].plot(t, active_model[7, :] / scaley, color='gold', label='Age Group %s ' % 0)
    axs[7].fill_between(t, active_model[7, :] / scaley - 5 * 4 / 10, active_model[7, :] / scaley + 5 * 4 / 10,
                        facecolor=colors_fill[0])
    # axs[7].set_title('Age Group A70-79')

    axs[8].plot(t, active_real[8, :] / scaley, color='darkslategray', label='Age Group %s ' % 0)
    axs[8].plot(t, active_model[8, :] / scaley, color='gold', label='Age Group %s ' % 0)
    axs[8].fill_between(t, active_model[8, :] / scaley - 5 * 4 / 10, active_model[8, :] / scaley + 5 * 4 / 10,
                        facecolor=colors_fill[0])
    # axs[8].set_title('Age Group A80+')
    ...
    plt.legend(["Real Active Infections","Simualated Active Infections"], fontsize=20)

    plt.xticks(np.arange(1, N + 1, step=4), months_2, rotation=30, fontsize=14)
    plt.yticks(fontsize=14)
    # plt.ylabel(r'Active Infections $\cdot 10^3$', fontsize=14)
    plt.xlabel(r"Months", fontsize=20)
    plt.xticks(np.arange(1, N + 1, step=4), months, rotation=30, fontsize=20)
    plt.grid(b=True, which='major', color='lightgray', linestyle='-')
...
def plot_deaths_sim(t, dead, age_groups, N, bounds):
    ...
    for i in range(age_groups):
        ...
        plt.fill_between(t, dead[i, :]/ scaley-5*i/200, dead[i, :]/ scaley+5*i/200, facecolor=colors_fill[i])
    plt.legend(["A 00-09", "A 10-19", "A 20-29", "A 30-39", "A 40-49", "A 50-59", "A 60-69", "A 70-79","A  80+"], fontsize=14)
    if (N==20):
        plt.xticks(np.arange(1, N +1, step=4), months, rotation=30, fontsize=14)
    elif(N==17):
...
def plot_deaths_real(t, dead, age_groups, N):
    plt.title("Total Deaths Germany (Real Data)", fontsize = 14)
    colors = ["navy", "forestgreen", "darkgoldenrod", "brown", "purple","black", "red", "peru", "darkgoldenrod", "olive","darkolivegreen"]
    colors_fill = ["mediumslateblue", "springgreen", "peachpuff", "lightpink", "lavender","pink", "moccasin", "wheat", "beige", "thistle"]
    scaley = 1e3
    for i in range(age_groups):
...
def plot_cumulative(male, female, diverse):
    ...
    fig = plt.figure()
    plt.title("Cumulative Infections in Germany on 06/12/2020", fontsize = 14)
    plt.yticks(fontsize = 14)
    ...
    ax.set_ylabel(r'Cumulative Infections [10^3]$', fontsize = 14)
...
def plot_betas_diag(betas, N, size_one, age_groups, start):
    ...
    for i in range(N-1):
      ...
      # max_abs = np.max(np.abs(beta_t))
      plt.figure()
      plt.title("Transmission Rate Matrix  Calendar Week " + str(start + i), fontsize = 14)
...
def plot_betas_tv(t, betas, N, size_one, age_groups):
    ...
    plt.yticks(fontsize=14)
    plt.ylabel("Value of Transmission rate", fontsize=14)
    plt.xticks(np.arange(1, N + 1, step=4), months, rotation=30, fontsize=14)
    plt.xlabel("Months", fontsize=14)
...
def plot_betas(t, betas, run_id, ub_betas, lb_betas,N, dataset):
    ...
    plt.title("Transmission rate " + run_id)
...
def plot_state_trajectories(t, S_model, I_model, R_model, D_model, S_real, I_real, R_real, D_real, N1, all_S_ub, all_S_lb,
    ...
    plt.suptitle("State Trajectory Estimations")
...
def show_cov(sigma_theta, max_abs, dataset):
    ...
    plt.colorbar()
    plt.title("Covariance Matrix of the Parameter Estimates")
...
def plot_betas_matix(betas, contact_matrix,N,  age_groups, start):
    max_abs = np.max(np.abs(contact_matrix))
    # for i in range(N - 1):
    #     plt.figure()
    #
    #     # plt.imshow(betas[i]*contact_matrix, cmap='RdBu', vmin=-max_abs, vmax=max_abs)
    plt.figure()
    plt.imshow(contact_matrix, cmap='RdBu', vmin=-max_abs, vmax=max_abs)
    plt.colorbar()
    plt.title(r"Contact Matrix", fontsize=14)
    plt.savefig(os.getcwd() + "/figures/coupling/new_data/contact_matrix.pdf")
    plt.xlabel(r"Age Groups", fontsize=14)
    plt.ylabel(r"Age Groups", fontsize=14)

    plt.xticks(ticks=np.arange(age_groups), labels=["A 00-09", "A 10-19", "A 20-29", "A 30-39", "A 40-49", "A 50-59", "A 60-69", "A 70-79", "A  80+"], rotation=30,
               fontsize=14)
    plt.yticks(ticks=np.arange(age_groups), labels=["A 00-09", "A 10-19", "A 20-29", "A 30-39", "A 40-49", "A 50-59", "A 60-69", "A 70-79", "A  80+"], rotation=30,
               fontsize=14)
        # plt.colorbar()
    plt.savefig(os.getcwd() + "/figures/coupling/new_data/contactmatrix.pdf", bbox_inches='tight')
...
def plot_betas(t, betas, N):
    ...
    plt.title(r'Values of $\beta$ using $\bar{\alpha} = 10^8$', fontsize=18)

    plt.grid(b=True, which='major', color='lightgray', linestyle='-')

    plt.xticks(np.arange(0, N + 1, step=4), months, rotation=30, fontsize=14)
    # plt.ylim((0, 1.7))
    plt.xlabel("Months", fontsize=14)
    ...
    plt.savefig(os.getcwd() + "/figures/coupling/new_data/betas.pdf", bbox_inches='tight')

...
for i in range(0,age_groups):
  for j in range(N):
    ...
    final_data[i+age_groups][j] = active[i][j]
    final_data[i+2*age_groups][j] = recovered[i][j]
    final_data[i+3*age_groups][j] = dead[i][j]
...
dSdt, dIdt, dRdt, dDdt = sird_model_sym_re_9(states, beta ,params, beta_tv)
rhs = vertcat(dSdt, dIdt, dRdt, dDdt)
# Form an ode function

ode_age = Function('ode_age',[states, beta, beta_tv, params],[rhs])

############ Creating a simulator ##########
N_steps_per_sample = 1
dt = 0.1

states_final_age = rk4_age(ode_age, states, beta, beta_tv, params, dt)
one_step_age = Function('one_step_age',[states, beta, beta_tv, params],[states_final_age])

X_age = states

for i in range(N_steps_per_sample):
  X_age = one_step_age(X_age, beta,beta_tv, params)

one_sample_age = Function('one_sample_age',[states, beta,  beta_tv, params], [X_age])
...
for i in range(N-1):
  current_control = u_time_var[0:age_groups*age_groups]
  current_control_t = reshape(current_control,age_groups,age_groups)
  beta_control = u_time_var[age_groups*age_groups+i: age_groups*age_groups + (i+1)]
  xcurrent= one_sample_age(xcurrent, current_control_t, beta_control, params)
  x_traj[:, i+1] = xcurrent
...
lbg_age = vertcat(lbg_age, 1)
ubg_age = vertcat(ubg_age,1)
...
lbg_all = vertcat(lbg_age, 1)
ubg_all = vertcat(ubg_age, ubg_sigma , ubg_mu,1)
...
current_control_t = reshape(current_control,age_groups,age_groups)
for i in range(9):
    g_betas = g_betas + current_control_t[0,i]

g_all = vertcat(g_age,g_betas)


all_betas = u_test[age_groups*age_groups:age_groups*age_groups+N-1]
current_control = u_test[0:age_groups*age_groups]

reg = symbol('reg', N-2  + age_groups*age_groups-1)
for i in range(np.shape(all_betas)[0]-1):
    reg[i] = all_betas[i+1] - all_betas[i]
for i in range(np.shape(all_betas)[0]-1, np.shape((reg))[0]):
    reg[i] = current_control[i-35] - current_control[i+1-35]
#TODO: FIX Regularization
# nlp_age = {'x':u_test, 'f':dot(model_mismatch, model_mismatch) + alpha_reg*dot(reg,reg),'g':g_age}
pdb.set_trace()
nlp_age = {'x':u_test, 'f':dot(model_mismatch, model_mismatch) + alpha_reg*dot(reg,reg),'g':g_all}

solver_age = nlpsol("solver", "ipopt", nlp_age)   # DM(1.95599)

u_guess_age = np.ones(np.shape(u_test))/9
...
sigma_theta = np.sqrt(sigma_theta)
sigma_theta = np.sqrt(sigma_theta)
sigma_theta = np.sqrt(sigma_theta)/50
...
f1 = y_sym[0:age_groups,:]
f2 = y_sym[age_groups:2*age_groups,:]
f3 = y_sym[2*age_groups:3*age_groups,:]
f4 = y_sym[3*age_groups:4*age_groups,:]
...
sigma_I = np.reshape(sigma_I,(age_groups,N))
sigma_D = np.reshape(sigma_D,(age_groups,N))
...
contact_matrix = solution_params[0:9*9]
betas = solution_params[9*9:9*9 + N-1]
gamma = solution_params[9*9 + N-1 : 9*9 + N-1+age_groups]
mu = solution_params[9*9 + N-1+age_groups : 9*9 + N-1 + 2*age_groups]

current_contact = reshape(contact_matrix, age_groups, age_groups)

for i in range(N-1):

  beta_current = betas[i:i+1]
  xcurrent = one_sample_age(xcurrent, current_contact, beta_current, vertcat(gamma.T,mu.T))

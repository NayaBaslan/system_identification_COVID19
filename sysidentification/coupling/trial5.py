...
df7 = pd.read_excel(os.getcwd() + '/sysidentification/data/male_IRD_red.xlsx')
df8 = pd.read_excel(os.getcwd() + '/sysidentification/data/age_groups_pop_germany_red.xlsx')
...
male_active2 = df7.iloc[124:124+5, start:start + N]
male_active2 = male_active2.to_numpy()
male_recovered2 = df7.iloc[140:140+5, start:start + N]
male_recovered2 = male_recovered2.to_numpy()
male_dead2 = df7.iloc[132:132+5,  start:start + N]
male_dead2 = male_dead2.to_numpy()

male_pop_ger_age = df8.iloc[10:15, 7]
...
for i in range(0,age_groups):
  for j in range(N):
    ...
    final_data[i+5][j] = active_age[i][j]
    final_data[i+10][j] = recovered_age[i][j]
    final_data[i+15][j] = dead_age[i][j]
...
for i in range(len(dead_init_age)):
  ...
  x0[i+age_groups] = infected_init_age[i]
  x0[i+2*age_groups] = recovered_init_age[i]
  x0[i+3*age_groups] = dead_init_age[i]

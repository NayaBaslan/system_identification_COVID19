...
plt.xticks(np.arange(1, N -10, step=4), months, rotation=30, fontsize=14)
...
for i in range(len(age_groups)):
    for j in range(len(weeks)):
        text = ax.text(j, i, (int)(active[i, j]/1000),
                       ha="center", va="center", color="w")

# ax.set_title("Active Infections Germany ")

ax.set_title(r' Active Infections Germany $\cdot 1e3$', fontsize=14)
# pdb.set_trace()
# fig.tight_layout()
# import pdb ; pdb.set_trace()
plt.savefig(os.getcwd() + "/figures/coupling/new_data/heatmap_active.pdf", bbox_inches='tight')

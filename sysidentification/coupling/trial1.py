...
u_time_var2 = u_time_var2 + u_time_var2.T - diag(diag(u_time_var2))
u_time_var2 = reshape(u_time_var2, 1,-1)

for i in range(1,N-1):
    new_control = symbol('u{0}'.format(i),Sparsity.lower(9))
    new_control = new_control + new_control.T - diag(diag(new_control))
    new_control = reshape(new_control, 1, -1)
    u_time_var2 = vertcat(u_time_var2 , new_control)

# u_time_var = u_time_var2.T
...
betas_control = beta = symbol('beta_controls', size)
betas_control2 = []
...
for i in range(N-1):
  betas = all_betas[i * 81:i * 81 + 81]
  betas_one_step = reshape(betas, 9, 9)
  for k in range(9):
      for j in range(k+1):
          betas_control2.append(betas_one_step[k,j])
          print("yesy", betas_one_step[k,j])


for i in range(size):
    betas_control[i] = betas_control2[i]

u_test = vertcat(betas_control, gamma.T, mu.T)
########################################################################
pdb.set_trace()
